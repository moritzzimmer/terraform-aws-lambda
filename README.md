...
| <a name="input_logging_config"></a> [logging\_config](#input\_logging\_config) | The logging configuration of the Lambda function. See the `logging_config` block arguments for details. | <pre>object({<br/>    application_log_level = optional(string)<br/>    log_format            = optional(string)<br/>    system_log_level      = optional(string)<br/>  })</pre> | `null` | no |
